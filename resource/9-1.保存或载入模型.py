...
# 如果想每训练一代（epoch)就想保存一次模型，则可以将 max_to_keep设置为None或者0，但是这样做除了多占用硬盘，并没有实际多大的用处，因此不推荐
# 保存最后一代的模型，则只需要将max_to_keep设置为1即可max_to_keep=1
...
with tf.Session() as sess:
    ...
    if isTrain:
        ...
        for step in range(steps_num):
            ...
            # 执行训练 (keep_prob:调整dropout参数，1.0代表100%的神经元工作，0.75代表75%的神经元工作)
        ...
        # 保存训练模型 第三个参数(global_step)将训练的次数作为后缀加入到模型名字中。
        # saver.save(sess, 'my-model', global_step=0) ==>      filename: 'my-model-0'
        # saver.save(sess, 'my-model', global_step=1000) ==> filename: 'my-model-1000'
        saver.save(sess, DIR + 'projector_cnn/net/cnn_net.ckpt', global_step=steps_num)
